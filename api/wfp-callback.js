...
export default async function handler(req, res) {
  ...
  /* WayForPay шле JSON, але Content-Type у них буває
     form-urlencoded — тоді все тіло лежить єдиним ключем. */
  let body = req.body;
  if (typeof body === 'string') {
    try { body = JSON.parse(body); } catch { body = {}; }
  }
  if (body && !body.orderReference) {
    const only = Object.keys(body)[0];
    if (only && only.trim().startsWith('{')) {
      try { body = JSON.parse(only); } catch { /* лишаємо як є */ }
    }
  }
  ...
  if (!orderReference) return res.status(400).end();
  ...
  if (approved && paidEnough) {
    ...
    await admin.from('subscriptions').upsert({
      ...
      /* `trialing`, а не `active`: доступ той самий, але в
         ...
         саме спишуться гроші. Людина, яку списання заскочило
         зненацька, не продовжує підписку — вона пише в підтримку. */
