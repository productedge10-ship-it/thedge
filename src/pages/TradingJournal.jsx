...
import {
  AssetFilter, PeriodFilter, periodToRange,
  QuickFilters, QUICK,
...
function ChartTooltip({ active, payload }) {
  ...
  const rows = [
    { label: 'Накопичено', value: `${d.cumulativeRR > 0 ? '+' : ''}${d.cumulativeRR}R`, color: d.cumulativeRR >= 0 ? T.ok : T.bad, big: true },
    { label: 'Гроші',      value: `${d.cumulativeProfit > 0 ? '+' : d.cumulativeProfit < 0 ? '−' : ''}$${Math.abs(d.cumulativeProfit).toFixed(2)}`, color: d.cumulativeProfit >= 0 ? T.ok : T.bad },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-[12px] font-bold tabular-nums" style={{ fontFamily: T.sans, color: T.text4 }}>
          угода №{d.trades}
        </span>
      </div>
      ...
      <div className="flex flex-col gap-2 px-3.5 py-3">
        {rows.map((r) => (
          <div key={r.label} className="flex items-baseline justify-between gap-6">
            ...
            <span
              className={`${r.big ? 'text-[16px]' : 'text-[14px]'} font-bold tabular-nums`}
...
export default function TradingJournal() {
  ...
  const { from: dateFrom, to: dateTo } = useMemo(() => periodToRange(period), [period]);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data) {
        const map = {};
        data.forEach((a) => {
          map[a.firm_name] = parseFloat(a.account_size || a.balance || a.size || a.amount || 0);
        });
        setAccountsMap(map);
      }
    })();
    ...
    (async () => {
      ...
      if (data) setUniquePairs(['All', ...new Set(data.map((t) => t.plan_pair).filter(Boolean))]);
    })();
  }, []);
  ...
  const fetchGlobalData = useCallback(async () => {
    const q = applyFilters(
      supabase.from('trades')
        .select('plan_date, result, rr, followed_plan, has_mistake, rushed, account_name, risk')
  ...
  const fetchTradesList = useCallback(async (reset = false, currentLen = 0) => {
    reset ? (setLoadingInitial(true), setHasMore(true)) : setLoadingMore(true);
    try {
      const from = reset ? 0 : currentLen;
      const q = applyFilters(
        supabase.from('trades').select('*').order('plan_date', { ascending: false })
  ...
  const stats = useMemo(() => {
    ...
    let wins = 0, totalRR = 0, totalProfit = 0, followed = 0, mistakes = 0, rushed = 0;
  ...
  const chartData = useMemo(() => {
    let cumRR = 0, cumProfit = 0, wins = 0, followed = 0;
  ...
  const quickCounts = useMemo(() => {
    const c = {};
    QUICK.forEach((f) => { c[f.id] = trades.filter(f.test).length; });
    return c;
  }, [trades]);
  ...
  return (
    <div className="relative min-h-screen w-full">
      ...
      >
        {/* ─────────── Хедер ─────────── */}
        <motion.div variants={fadeUp} className="mb-6 flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <AssetFilter options={uniquePairs} value={filterPair} onChange={setFilterPair} />
        ...
        >
          <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${T.line}` }}>
            <div className="flex items-center gap-3">
              ...
              >
                <TrendingUp size={17} strokeWidth={2.3} style={{ color: T.acc }} />
              </div>
              <div>
                <h3 className="text-[17px] font-bold leading-tight" style={{ fontFamily: T.display, color: T.text }}>
                  Крива капіталу
                </h3>
                <p className="mt-1 text-[13px]" style={{ color: T.text3, fontFamily: T.sans }}>
                ...
                <AreaChart data={chartData} margin={{ top: 4, right: 16, left: -14, bottom: 0 }}>
                  <defs>
                    <linearGradient id="jrnRR" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor={rrUp ? T.ok : T.bad} stopOpacity={0.20} />
                      <stop offset="100%" stopColor={rrUp ? T.ok : T.bad} stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  ...
                  <Area
                    ...
                    dot={false}
                    activeDot={{ r: 4, fill: rrUp ? T.ok : T.bad, stroke: T.surface, strokeWidth: 2 }}
                  />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
        ...
        >
          <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: `1px solid ${T.line}` }}>
            <div
              className="grid h-10 w-10 place-items-center rounded-xl"
              style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${T.line}` }}
            >
              <BookOpen size={17} strokeWidth={2.3} style={{ color: T.text3 }} />
            </div>
            <div>
              <h3 className="text-[17px] font-bold leading-tight" style={{ fontFamily: T.display, color: T.text }}>
                Угоди
          ...
          <QuickFilters
            ...
            total={trades.length}
            onToggle={(id) => setQuick((q) => (q.includes(id) ? q.filter((x) => x !== id) : [...q, id]))}
            onClear={() => setQuick([])}
          />
      ...
      <AnimatePresence>
        {tradeToDelete && (
          <motion.div
            ...
            style={{ background: 'rgba(6,6,8,0.82)', backdropFilter: 'blur(12px)' }}
          >
            <motion.div
              ...
              style={{ background: T.surface, border: `1px solid ${T.lineHi}`, boxShadow: '0 30px 70px rgba(0,0,0,0.8)' }}
            >
              <div className="flex flex-col items-center gap-3 px-6 pb-2 pt-7 text-center">
                ...
                >
                  <AlertTriangle size={18} strokeWidth={2.3} style={{ color: T.bad }} />
                </div>
                <h3 className="text-[18px] font-bold" style={{ fontFamily: T.display, color: T.text }}>
                  Видалити угоду?
                </h3>
      ...
      <AnimatePresence>
        {selectedTrade && (
          <TradeDetailsModal
            ...
            onUpdated={() => { fetchGlobalData(); fetchTradesList(true); }}
          />
        )}
      </AnimatePresence>
    </div>
