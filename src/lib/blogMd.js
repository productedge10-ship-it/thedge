/* ==================================================================
   ...
   Підтримується:
     ...
     ---             лінія
     ```             блок коду
   Усередині рядка: **жирне**, *курсив*, ==підсвітка==,
     [текст](адреса), `код`.
================================================================== */
...
export const inline = (text) => {
  ...
  return parts.map((chunk) => {
    ...
    /* ==підсвітка==: єдиний спосіб пофарбувати текст у статті.

       Довільного кольору немає навмисно. Стаття верстається однією
       типографічною системою, і автор, який може взяти будь-який
       колір, рано чи пізно візьме той, що не читається на темному
       тлі або свариться з кольором розділу. Одна підсвітка акцентом
       закриває реальну потребу і не дає зіпсувати сторінку. */
    if (chunk.startsWith('==') && chunk.endsWith('==') && chunk.length > 4) {
      return { t: 'mark', text: chunk.slice(2, -2) };
    }
