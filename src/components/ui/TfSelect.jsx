  ...
  return (
    <div className="relative z-50" ref={ref}>
      ...
      >
        ...
        <span className="text-xs font-bold uppercase leading-none tracking-[0.15em] text-[var(--edge-text2)] transition-colors group-hover:text-[var(--edge-text)]">
          {value || 'TF'}
        </span>
        <motion.span
          ...
          className="flex"
        >
          <ChevronDown size={13} className="text-[var(--edge-text3)] transition-colors group-hover:text-[var(--edge-text2)]" />
        </motion.span>
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute left-0 top-[calc(100%+8px)] w-[240px] origin-top-left rounded-2xl border border-[var(--edge-hair-strong)] bg-[var(--edge-panel)] p-2.5 shadow-[0_20px_50px_-10px_var(--edge-panel-glow,rgba(0,0,0,0.5))] backdrop-blur-2xl"
          >
            {TF_GROUPS.map((group, gi) => (
              <div key={group.label} className={gi > 0 ? 'mt-3' : ''}>
                <div className="px-1.5 pb-1.5 text-[9px] font-bold uppercase tracking-[0.2em] text-[var(--edge-text4)]">
                  {group.label}
                </div>
