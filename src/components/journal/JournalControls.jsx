...
const CTRL = 'flex h-[42px] items-center gap-2 rounded-xl px-4 text-[14px] font-bold transition-all duration-200';

/* ---------- Дропдаун активу ---------- */
export function AssetFilter({ options, value, onChange }) {
    ...
    <div className="relative" ref={ref}>
      ...
      >
        <span className="flex items-center gap-2">
          <Filter size={14} strokeWidth={2.5} style={{ color: value === 'All' ? T.text4 : T.acc }} />
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute left-0 top-[calc(100%+6px)] z-[120] max-h-[280px] w-full min-w-[180px] overflow-y-auto rounded-xl p-1.5 custom-scrollbar"
            style={{ background: T.surface, border: `1px solid ${T.lineHi}`, boxShadow: '0 20px 50px rgba(0,0,0,0.8)' }}
          >
            {options.map((o) => {
              ...
              return (
                ...
                >
                  <span
                    ...
                    style={{ fontFamily: T.sans, color: active ? T.acc : T.text2 }}
                  >
                    {o === 'All' ? 'Усі активи' : o}
                  </span>
                  {active && <Check size={14} strokeWidth={3} style={{ color: T.acc }} />}
                </button>
              );
            })}
          </motion.div>
...
export function PeriodFilter({ value, onChange }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <span className="flex items-center gap-2">
          <Calendar size={14} strokeWidth={2.5} style={{ color: value === 'all' ? T.text4 : T.acc }} />
          {current.label}
        </span>
        <motion.span animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }} className="flex">
          <ChevronDown size={14} strokeWidth={2.5} style={{ color: T.text4 }} />
            ...
            {PERIODS.map((p) => {
              ...
              return (
                <button
                  ...
                  onMouseLeave={(e) => (e.currentTarget.style.background = active ? T.surfaceHi : 'transparent')}
                >
                  <span className="text-[14px] font-bold" style={{ fontFamily: T.sans, color: active ? T.acc : T.text2 }}>
                    {p.label}
                  </span>
                  {active && <Check size={14} strokeWidth={3} style={{ color: T.acc }} />}
                </button>
...
export function QuickFilters({ active, onToggle, onClear, counts, shown, total }) {
  const has = active.length > 0;

  return (
    <div className="flex flex-wrap items-center gap-2 px-5 py-3" style={{ borderBottom: `1px solid ${T.line}` }}>
      {QUICK.map((f) => {
        ...
        return (
          ...
          >
            {f.label}
            <span className="tabular-nums opacity-60" style={{ fontFamily: T.mono }}>{n}</span>
          </motion.button>
        );
      })}
