...
- **Один тріал на людину**: таблиця `trial_marks` (`2026-09-28_trial_marks.sql`, лише sha256, з браузера недоступна). `mono-pay` не дає тріал на одноразові скриньки й на пошту, з якої вже брали (Gmail — без крапок і `+тегів`, `normEmail`); `applyInvoice` при привʼязці картки перевіряє маску PAN — чужа картка → 1 ₴ назад, тріалу немає, на екрані підписки `trialDenied`; **але mono віддає лише 2 останні цифри (`43260979******08`), а `cardMark` вимагає 4, тож для mono перевірка картки зараз фактично вимкнена** (інакше хибні збіги між клієнтами одного банку). MT5: тригер `mt5_trial_guard` на `mt5_accounts` (`2026-09-28_trial_mt5.sql`) не дає на `trialing` підключити рахунок (сервер+логін), уже підключений на тріалі з іншого акаунта: з `2026-09-29_trial_mt5_close.sql` тригер **закриває тріал повністю** (free/inactive, `next_charge_at = null`, `subscriptions.trial_block = 'mt5'`) і тихо не записує рядок (`return null` — помилка відкотила б закриття); `connectMt5` бачить порожній результат + `trial_block` і кидає `trial_mt5_used`, `SyncStatus` показує текст і кнопку Telegram, `SubscriptionTab` — пояснення (`trialBlock`). **Купівля лише з підтвердженою поштою** (`profiles.email_verified`, перевіряють `mono-pay` і `np-pay`); решта застосунку без підтвердження, модалка `VerifyEmailModal` сама не вискакує.
